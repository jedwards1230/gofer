...
func TestPanelConfigEditPersistsViaSaveConfig(t *testing.T) {
	...
	// tools.schema_mode, search.provider, telemetry.enabled — eleven ↓
	// presses (the first selects row 0) land on telemetry.enabled, a bool row
	// that saves on Enter with no further input needed. This package can't
	// read the unexported registry to derive the count (config_view_test.go's
	// downPressesTo does), so the row list above is the thing to update when
	// a setting is added ahead of telemetry.enabled — the assertions below
