...
// enum mapping for every state plus an unrecognized value. It is an
...
// can never exercise that branch or the unrecognized-value fallback. "idle" is
// the reloaded/at-rest status a resumed session reports — it must map to its own
// enum value, not fall through to the StatusNeedsInput default (which would put a
// browsed reloaded session back on the awaiting-input counter).
