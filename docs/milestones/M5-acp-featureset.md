...
Slice PRs target `main`. The original `milestone/m5-acp-featureset` integration branch merged via #100 and has been deleted; remaining slices land directly.
...
- [ ] **Slice 3 — Session methods** — **gofer `session/set_config_option` dispatch + `session/list` confirmation DONE** (agent-sdk-go v0.7.0): `set_config_option` `configId=model` maps to the gofer-native `set_model` path and replies a `model` select option (registry catalog + current value); unknown `configId` → rpc error. `session/list` was already wired and still holds on the v0.7.0 types. **Resume DONE** — `session/load` replays full history per ACP v1 (`internal/daemon/handlers.go`), and the router spawns a fresh worker for an offline id (M6 Phase 4, #139). Remaining: Agmente client.
- [ ] **Slice 4 — Model discovery** — **gofer half DONE**: the gofer-native `gofer/models` list-models endpoint over the provider registry is on `main` (`internal/daemon/handlers.go`, `methodGoferModels`/`handleGoferModels`, with `models_test.go` + `wire_models_internal_test.go`). Remaining: the Agmente picker (Agmente repo). `set_model` stays gofer-native.
...
- [x] **TUI — transcript word-wrap** — SHIPPED. Chat/message body wraps to width instead of truncating with `…`: `transcriptLines` runs each rendered line through `wrap` (`internal/tui/model.go:467-469`, `wrap` at `:814` over `ansi.Wrap`). List/status/roster rows stay clamped on `truncate`, as intended.

## In this branch now (parallel)

1. **gofer-native list-models endpoint** — Slice 4's gofer half.
