...
> gofer milestone — **shipped.** Merged to `main` via #114 and 15+ follow-on slice PRs; Phases 0-3 and most of Phase 4 are complete (§11). Detached per-session worker processes behind a thin router daemon, so the daemon and CLI can be upgraded in place while live turns finish uninterrupted on the binary that started them. Two Phase-4 polish items remain open: offline `resume` spawning a fresh worker ([#139](https://github.com/jedwards1230/gofer/issues/139)) and cost/usage aggregation + graceful drain ([#140](https://github.com/jedwards1230/gofer/issues/140)).
...
**Spawn & detachment.** On `session/new`, the router forks/execs `gofer session-worker --session <uuid> --root <root> [--model <model>]` (`internal/router/router.go:612`, `buildWorkerCmd`). *Correcting an earlier claim in this section: there is no `--listen` flag.* The worker derives its own unix-socket path from the pinned session id (`daemon.WorkerSocketPath`, `internal/worker/worker.go:203`), binds it, and advertises the address by writing the `<uuid>.json` endpoint file (§4) — the router discovers it via a short poll loop (`internal/router/router.go`'s `waitForWorkerEndpoint`), not by reading it back from argv. The worker is detached so it outlives the router: `SysProcAttr{Setsid: true}` (new session + process group, no controlling terminal), stdio redirected to a per-worker log file. Unix-only, matching the repo's posture (no Windows build — see `pidAlive`'s comment).
...
> **Adoption-time pure-ACP exception (still open):** the standing per-adopted-session watcher re-fans a re-surfaced permission only as the gofer-native `gofer/permission_requested` notification (and records its call-id→session route, so *any* client's routed `permission.reply` resolves it). It does **not** re-issue the spec-ACP `session/request_permission` request, so a pure-ACP peer (a phone) still cannot *answer* a re-surfaced permission on an adopted session. *Correcting an earlier claim in this section: it scoped this gap to "through Phase 2," expecting Phase 3 to close it. Phase 3 (version skew + the hot-upgrade story, §11) has since shipped without doing so — `internal/daemon/permission_relay.go`'s doc comment, last touched after Phase 3 landed, still defers the ACP-ask wiring to an unscheduled future phase.* gofer clients (TUI/daemonbridge) are unaffected.
...
*Correcting an earlier claim: only Phase 0 was marked shipped below. Phases 1-3 have since shipped too, and most of Phase 4 has — only the two items called out under Phase 4 remain open.*
