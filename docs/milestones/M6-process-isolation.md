...
> gofer milestone — **shipped**, behind the still-opt-in, off-by-default `gofer daemon --workers` flag. Merged to `main` via #114 and 15+ follow-on slice PRs. Detached per-session worker processes behind a thin router daemon, so the daemon and CLI can be upgraded in place while live turns finish uninterrupted on the binary that started them. **Phases 0-4 are now complete**: Phase 4's two headline items — offline `resume` spawning a fresh worker ([#139](https://github.com/jedwards1230/gofer/issues/139), closed) and cost/usage aggregation + graceful drain ([#140](https://github.com/jedwards1230/gofer/issues/140), closed) — have both landed. Only roster reconciliation edge cases (§11) remain unverified.
...
| `resume` | `runner.Resume` from journal | **Shipped** ([#139](https://github.com/jedwards1230/gofer/issues/139), closed). `Supervisor.Resume` reattaches a session the router already hosts live; for an id with no live handle it falls through to `confirmJournal` + `resumeOffline`, which spawns a **fresh** worker seeded via `session/load` (`internal/router/methods.go`). Note this is NOT the originally-designed `--resume <uuid>` flag — `session-worker` still has no such flag (`cmd/gofer/session_worker.go`), deliberately: the journal is replayed through the existing load path instead |
...
- **One new SDK seam WAS required and taken: `runner.Options.SessionID`** (shipped in SDK v0.11.0, consumed at `cmd/gofer/session_worker.go`). Option A (§4) has the router pre-generate a session's uuid to key the worker's socket, endpoint file and lock **before** the worker starts, so the worker must make its session adopt that exact id. gofer first bridged this with a stateful `IDGen` whose first draw returned the pinned uuid; the SDK then added `SessionID` specifically to replace that bridge, and the bridge was deleted. The seam is strictly better than what it replaced: it pins at session creation rather than "whenever the store happens to draw first," and it validates the id before it becomes a journal filename.
- **`event.Unmarshal` has landed in the SDK** (v0.11.0, `event/unmarshal.go`) — the canonical inverse of `MarshalJSON` this section anticipated. gofer has **adopted it**: `handleGoferEvent` calls `event.Unmarshal` (`internal/wirestream/reconstruct.go:516` — the reconstruction core was extracted to its own package during M6) and the hand-rolled per-kind switch was deleted, in [#346](https://github.com/jedwards1230/gofer/pull/346). `TestReconstructCarriesEveryEventKind` fails the build if a per-kind path is reintroduced.
...
Phases 0-4 have all shipped. Only roster reconciliation edge cases remain unverified.
...
- **Phase 4 — lifecycle completeness (polish) (shipped).** `resume` spawns a fresh worker — **done** ([#139](https://github.com/jedwards1230/gofer/issues/139), closed; `confirmJournal` + `resumeOffline` seed it via `session/load`); cost/usage aggregation across workers and graceful worker drain on `gofer daemon uninstall` — **done** ([#140](https://github.com/jedwards1230/gofer/issues/140), closed; drain is configurable via `drain_timeout_ms`); roster reconciliation edge cases — status still unverified.
...
Slots as the **next** gofer milestone after M5 (ACP featureset), pushing ecosystem and auto+polish back one. **No SDK *milestone* was required** — the SDK's Event/Op contract, two-tier broker, and journal already supported out-of-process reconstruction. M6 needed no SDK milestone, only two additive seams shipped in v0.11.0 (`runner.Options.SessionID`, adopted; `event.Unmarshal`, available but not yet adopted). Option A's id-pinning requirement is what made the `SessionID` seam necessary — see §9.
