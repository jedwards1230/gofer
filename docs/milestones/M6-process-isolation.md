...
- The router reconstructs each session's typed stream by decode-dispatch-publish into a per-session broker via the SDK's `event.New*` constructors — **exactly `daemonbridge/reconstruct.go:handleGoferEvent` today**. Attach/peek fidelity, the history-before-live ordering guarantee, and the `TurnFinished`-vs-last-event ordering all carry over because the reconstruction and its ordering proof are already written and tested (`daemonbridge/fidelity_test.go`).
...
- **No new SDK seam is required.** gofer already reconstructs the typed event stream across a process boundary *in gofer* (`daemonbridge`), with no SDK dependency beyond the exported `event.New*` constructors and `MarshalJSON`. The version handshake is gofer-native (`gofer/hello`), owned by gofer the same way `gofer/event` is — so it needs no SDK change either.
- **One optional SDK promotion**, honoring the membership test: a canonical `event.Unmarshal([]byte) (event.Event, error)` — the inverse of `MarshalJSON`, today hand-rolled as `daemonbridge`'s per-kind switch. A second cross-process SDK consumer would need that exact decoder unchanged, so it passes the gate. **But gofer can and should ship without blocking on it** (keep the hand-rolled decoder; it works and is tested), and propose the promotion on the SDK's own schedule.
...
Slots as the **next** gofer milestone after M5 (ACP featureset), pushing ecosystem and auto+polish back one. **No SDK milestone is required** — the SDK's Event/Op contract, two-tier broker, and journal already support out-of-process reconstruction (proven by `daemonbridge`). The only SDK-side item is the *optional* incremental `event.Unmarshal` promotion (§9), which gofer does not block on.

**Honors the invariants:** contract-only SDK consumption (the boundary is the existing typed wire); everything-is-a-client (the router↔worker hop *is* an ACP+gofer client relationship — the worker gets no privileged path, and neither does the router); journals never deleted (they become the adoption/resume source of truth); SDK-promotion membership test applied to the one optional seam.

---

**Implementation pointers (code-verified):** boundary seam is `internal/supervisor/types.go:59` (`Session` interface) + `supervisor.go:50-54` (injectable factories); the reusable cross-process template is `internal/daemonbridge/reconstruct.go:handleGoferEvent`; the fan-out to preserve is `internal/daemon/handlers.go:broadcastGoferEvent`/`broadcastPermission`; the adoption primitives to mirror are `cmd/gofer/daemon.go:pidAlive`/`guardLiveEndpoint` + `internal/daemon/endpoint.go`.
